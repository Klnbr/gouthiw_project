import { FlatList, StyleSheet, Text, View, Image, SafeAreaView, Pressable, Alert } from 'react-native'
...
const UserDinner = ({ menuDinner }) => {
     ...
     const handledelete = async (itemId) => {
          try {
               const response = await axios.delete(`http://192.168.56.1:5500/dinner/delete/${itemId}`);
               ...
               console.log('Error deleting dinner', error);
     ...
     return (
          <View>
               <FlatList data={menuDinner} renderItem={({ item }) => {
                    const compareData = menu.find((menu) => menu?.menuName === item?.menuName);
